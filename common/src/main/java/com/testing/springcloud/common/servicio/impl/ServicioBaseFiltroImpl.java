...
public abstract class ServicioBaseFiltroImpl<ID, MODELO extends Modelo<ID>, ENTIDAD extends IdEntidad<ID>, FILTRO extends FiltroBase>
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public Stream<MODELO> buscarTodo(FILTRO filtro) {
		return buscarPorFiltro(filtro).map(obtenerConversionBase()::rightToLeft);
	}
	...
	protected abstract Stream<ENTIDAD> buscarPorFiltro(FILTRO filtro);
